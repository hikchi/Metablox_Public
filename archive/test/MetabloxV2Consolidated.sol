...
// File: contracts/storage/PropertyTier.sol
...
// File: contracts/storage/ERC3664/IERC3664.sol
...
// File: contracts/storage/ERC3664/extensions/IERC3664Metadata.sol
...
// File: contracts/storage/ERC3664/ERC3664.sol
...
// File: contracts/storage/PropertyLevel.sol
...
// File: contracts/v2/MetabloxV2.sol
    ...
    PausableUpgradeable
{
    ...
    function capBloxBeforePayment(uint _bloxId) public onlyOwner {
        require(!_exists(_bloxId), "the blox has been minted");
        cappedBlox[_bloxId] = true;
    }

    function capBloxAfterPayment(uint _bloxId) public onlyOwner {
        cappedBlox[_bloxId] = false;
    }
    ...
    function setCappedBloxes(uint[] memory _bloxIds, bool _flag) public onlyOwner {
    ...
    function batchMintReservedBloxes(
        ...
        require(_bloxIds.length == _uris.length, "unmatched length of blox id and uri");
        ...
        // mint execution
        for (uint i=0; i<_bloxIds.length; i++) {
            mintReservedBlox(_user, _bloxIds[i], _uris[i]);
        }
    }
    ...
    function batchMintBloxes(
        ...
        // check blox length availability
        require(
            _bloxIds.length == _uris.length &&
            _uris.length == _propertyTiers.length &&
        ...
        if (_bloxIds.length == 1) {
            mintBlox(_bloxIds[0], _uris[0], _propertyTiers[0], _buyWith, _erc20TokenAmounts[0], _tolerance);
        } else {
            for (uint i=0; i<_bloxIds.length; i++) {
                mintBlox(_bloxIds[i], _uris[i], _propertyTiers[i], _buyWith, _erc20TokenAmounts[i], _tolerance);
            }
        }
    }
