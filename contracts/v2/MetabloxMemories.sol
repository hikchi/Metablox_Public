...
import "./MetabloxController.sol";

contract MetabloxMemories is Ownable {
    IERC721 public bloxContract;
    MetabloxController private metabloxController;
    ...
    modifier onlyBloxController() {
        require(
            address(metabloxController) == msg.sender,
            "only metablox controller cna do this"
        );
        _;
    }
    ...
    function setBloxController(address _bloxController) external onlyOwner {
        metabloxController = MetabloxController(_bloxController);
    }
}
